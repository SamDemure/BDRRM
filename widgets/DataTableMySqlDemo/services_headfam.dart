...
class ServicesHeadfam {
  ...
  static Future<List<HeadOfFamily>> getHeadFamily() async {
    try {
      ...
      final response = await http.post(Uri.parse(ROOT), body: map);
      print('getHeadofFamily Response: ${response.body}');
      if (response.statusCode == 200) {
      ...
      return <HeadOfFamily>[]; // Return an empty list on exception/error
    }
  }
  ...
  // Method to add head of fam to the database
  static Future<String> addHeadofFamily(
    ...
    try {
      ...
      final response = await http.post(Uri.parse(ROOT), body: map);
      print('addHeadOfFamily Response: ${response.body}');
      if (response.statusCode == 200) {
  ...
  static Future<String> updateHeadofFamily(
    ...
    try {
      ...
      map['head_4psBeneficiary'] = isFourPsBeneficiary;
      final response = await http.post(Uri.parse(ROOT), body: map);
      print('updateHeadOfFamily Response: ${response.body}');
      if (response.statusCode == 200) {
        return response.body;
      } else {
        return 'error';
  ...
  static Future<String> deleteHeadofFamily(String headId) async {
    try {
      ...
      map['head_id'] = headId.toString();
      final response = await http.post(Uri.parse(ROOT), body: map);
      print('deleteHeadOfFamily Response: ${response.body}');
      if (response.statusCode == 200) {
        return response.body;
      } else {
        return 'error';
